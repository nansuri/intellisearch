...
- **Queue & limits** (`/admin/ai/queue`): edit `max_concurrent`, `max_queue_size`, `request_timeout_ms`, `per_user_rate_limit`, `suggestion_cache_hours` (how long the main page's AI suggestions are reused before recomposing), and `default_daily_quota` (daily AI usage granted to newly registered accounts — default 3, 0 = unlimited) with immediate apply — no redeploy.
...
- **Image results:** every web-search ask (both modes) also queries SearXNG's **images** category (`categories=images`) best-effort; the results are persisted as `image_results` per message and returned as `images` in the ask/session responses. The result page shows **Google-style tabs (All / Images)** under the query — All renders the answer + sources, Images a responsive grid (thumbnails linking to the originals, browser-loaded — the API never fetches third-party images). An image-search failure never fails an ask.
...
- `go test ./...`: passing (envelope + error-code format, AES-GCM round-trip, Ollama/OpenAI client mapping, SearXNG image-result parsing (thumbnail fallback, resolution), SSRF guard, crawl lifecycle, AI pipeline persistence + graceful degradation, queue overflow → `AISY02001`, rate limit → `AISY02002`, daily quota → `AISY02003`, logo upload/delete with default fallback, stats trends daily/weekly bucketing, **query masking (no raw query leaks into admin stats) + trending-words aggregation (per-bucket terms, stopword filtering, weekly window)**, search-history record/clear/suggestions + router coverage, history summaries (truncation, missing-message tolerance) + suggestion cache (TTL hit, force-refresh bypass, zero disables, clear invalidation) + queue-config validation including `default_daily_quota`, **register applies the default daily quota (fallback 3 + configurable value) + Google SSO users get it too**, register success/duplicate/weak-password + router coverage, Ollama models/health parsing + invalid-URL rejection + admin router coverage, search-vs-enhanced ask modes (search mode now composes an **extractive summary** from snippets — still no crawler, no provider attribution, history recorded — and both modes persist/return image results), session endpoint returns per-message `sources` + `images`, anonymous guest limit (first ask issues visitor + cookie, repeat token rejected → `AISY02004`, cleared-token still blocked by per-IP claim, fresh IP allowed, search mode free, URL ask consumes, signed-in user exempt, end-to-end router coverage)). Race detector clean.
