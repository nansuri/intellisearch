...
- **Queue & limits** (`/admin/ai/queue`): edit `max_concurrent`, `max_queue_size`, `request_timeout_ms`, `per_user_rate_limit`, and `suggestion_cache_hours` (how long the main page's AI suggestions are reused before recomposing) with immediate apply — no redeploy.
