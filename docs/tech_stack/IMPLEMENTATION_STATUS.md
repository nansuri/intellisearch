...
- **Image results:** every web-search ask (both modes) also queries SearXNG's **images** category (`categories=images`) best-effort; the results are persisted as `image_results` per message and returned as `images` in the ask/session responses. The result page renders them as a responsive **image grid** (thumbnails linking to the originals, browser-loaded — the API never fetches third-party images). An image-search failure never fails an ask.
- **Follow-up vs new search:** the result page labels the primary query **"New search"** and each thread entry **"Follow-up N"** with distinct badges; the header ask box shows a "Sending as a follow-up" pill with a **Start new search** action that clears the thread and returns to the main page.
