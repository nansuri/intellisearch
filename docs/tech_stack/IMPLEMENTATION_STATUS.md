...
- **`/admin/login`**: Super Owner–only sign-in gate; non–Super Owner accounts are routed to their own `/account` after login (role checks remain enforced on every `/admin/*` route), with a link to the general-user `/login` page.
...
- **AI providers** (`/admin/ai/providers`): create/edit/delete provider cards with type (Ollama / OpenAI-compatible), base URL (Ollama endpoint), model, JSON model parameters, encrypted API key (blank keeps existing), and single-click "set active" routing.
...
- **LLM provider abstraction** (`internal/services/llm_service.go`): Ollama (`/api/chat`) and OpenAI-compatible (`/v1/chat/completions`) clients; provider API keys decrypted at call time from AES-GCM storage.
