...
| POST | `/api/v1/auth/register` | public | Creates a general-user account and returns a signed JWT plus the profile (the user lands signed in). Body: `{ name, email, password }` — password must be ≥ 8 characters. Returns `AUTH01004` (400) for invalid data and `AUTH01005` (409) when the email is already registered. |
...
| POST | `/api/v1/ask` | public (rate-limited) | Submits a question to the AI pipeline (worker pool + bounded queue). Body: `{ query, sessionId?, location?, mode? }` where optional `location` is `{ latitude, longitude, accuracy? }` from the browser Geolocation API and `mode` is `"enhanced"` (default; SearXNG + crawler + LLM synthesis) or `"search"` (SearXNG results with an **extractive summary composed from the top snippets — no crawler, no LLM, zero AI tokens**). Every web-search ask also queries SearXNG's **images** category (best-effort). When the query mentions nearby intent (e.g. "near me", "near my place"), the backend reverse-geocodes the coordinates and enriches the SearXNG search query. Coordinates are not persisted in messages or usage logs. Response: `{ data: { sessionId, messageId, answer, sources, images, visitorId? } }` — `images: [{ position, title, url, thumbnailUrl, source?, width?, height? }]`. Follow-ups pass the `sessionId` to continue the same conversation. **Anonymous guest limit:** non-logged-in callers get exactly **one** AI usage (`mode=enhanced`) — see the rate-limiting section for the fraud-resistant enforcement. |
...
| GET | `/api/v1/sessions/:id` | public (anonymous sessions) or owner (signed-in sessions) | Returns one chat session with message history and per-message `sources` and `images` (persisted image results). Anonymous sessions (`userId` null) are readable by id without auth; owned sessions require the owning user. |
...
- **Graceful degradation:** if SearXNG is unreachable the LLM still answers without web sources. Every ask persists a `chat_sessions` row, user/assistant `messages`, `search_results` (source cards), `image_results` (image cards), and a `usage_logs` row with status/error/latency for the statistics panel.
- **Search mode is AI-free:** `mode=search` returns an **extractive summary** built from the top SearXNG snippets (no LLM call, no deep-read, no provider attribution in the usage log), so users can get a quick answer without consuming AI tokens. Image results are also returned/persisted in both modes; an image-search failure never fails the ask.
