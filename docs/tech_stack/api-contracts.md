...
| POST | `/api/v1/ask` | public (rate-limited) | Submits a question to the AI pipeline (worker pool + bounded queue). Body: `{ query, sessionId?, location?, mode? }` where optional `location` is `{ latitude, longitude, accuracy? }` from the browser Geolocation API and `mode` is `"enhanced"` (default; SearXNG + crawler + LLM synthesis) or `"search"` (raw SearXNG results only — no crawler, no LLM; `answer` comes back empty). When the query mentions nearby intent (e.g. "near me", "near my place"), the backend reverse-geocodes the coordinates and enriches the SearXNG search query. Coordinates are not persisted in messages or usage logs. Response: `{ data: { sessionId, messageId, answer, sources, visitorId? } }`. Follow-ups pass the `sessionId` to continue the same conversation. **Anonymous guest limit:** non-logged-in callers get exactly **one** AI usage (`mode=enhanced`) — see the rate-limiting section for the fraud-resistant enforcement. |
