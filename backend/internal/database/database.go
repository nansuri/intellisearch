...
func MigrateAndSeed(db *gorm.DB, cfg config.Config) error {
	...
	if err := seedSingleton(db, 1, &entities.AIQueueConfig{ID: 1, MaxConcurrent: 4, MaxQueueSize: 20, RequestTimeoutMS: 60000, PerUserRateLimit: 10, SuggestionCacheHours: 6}); err != nil {
		return err
	}
